...
class SurvAttachmentDialog extends StatefulWidget {
  final String title;
  const SurvAttachmentDialog({required this.title, super.key});

  @override
  SurvAttachmentDialogState createState() => SurvAttachmentDialogState();
}

class SurvAttachmentDialogState extends State<SurvAttachmentDialog> {
  PlatformFile? file;
  ...
  /// for final SurvAttachment data which is going to show user
  ...
  void loadResults(String title) {
    ...
    convertedContentList.add(generateSurvAttachmentRow(
        "Document Name", "Size", Colors.blue, 0, context));

    for (AttachmentData result in dbResults) {
      checklistIndex++;
      checklistValues.add(false);
      convertedContentList.add(generateSurvAttachmentRow(result.documentName!,
  ...
  Future<void> pickFilesFromDevice(BuildContext context) async {
    ...
    if (result != null) {
      ...
      for (PlatformFile platformfile in platformfiles) {
        ...
        attachmentData.documentName = platformfile.name;
        attachmentData.documentSize = "${platformfile.size / 1000} KB";
        attachmentData.color = Colors.black;
        dbResults.add(attachmentData);
      }
  ...
  Widget generateSurvAttachmentRow(String documentName, String sizeText,
