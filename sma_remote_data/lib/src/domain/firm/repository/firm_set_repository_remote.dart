...
part 'firm_set_repository_remote.g.dart';

@Riverpod(keepAlive: true)
class FirmSetRepositoryRemote extends _$FirmSetRepositoryRemote
  ...
  Future<int> createSet({required Stream<FirmInfo> stream}) async {
    var count = 0;
  ...
  FutureOr<Response<FirmInfo?>> create(FirmInfo remote) async {
    print("remote firm: $remote");
    return await _remoteConfig.anetSchema.firmPost(
  ...
  Stream<FirmInfo> fetchAfter(DateTime? date) async* {
    final pageSize = 10;
    int currentPage = 0;
...
//          "With EmployeeRole Id: ${employeeRole.id}, Emnployee Id: ${employeeRole.employeeId}, Id: ${employeeRole.roleId}, Region Code: ${employeeRole.regionCode}, Subregion Code: ${employeeRole.subRegionCode}");
      ...
      while (true) {
        ...
        final response = await _remoteConfig.anetSchema.firmAfterGet(
          date: date,
          page: currentPage,
        ...
        for (FirmInfo remote in records) {
          yield remote;
        }
